...
# y = [1, 2, 3, 4]
# x = [1, 2, 3, 4, 5]
# x_n, y_n = np.mgrid[1:5:250j, 1:4:200j]
...
#
def lmag2(temperatura, periodo_molhamento):
    lesao = 2.968 - 0.412 * temperatura - 0.105 * periodo_molhamento
    + 0.018 * pow(temperatura,  2) + 0.002 * temperatura * periodo_molhamento
    + 0.002 * pow(periodo_molhamento,  2)
    return abs(lesao)

# x_tk = [10, 15, 20, 25, 30]
x = [10, 15, 20, 25, 30]
y = [0, 12, 24, 36]
x_n, y_n = np.mgrid[10:30, 0:36]
x_prev, y_prev = np.meshgrid(x, y)

# -------------------------------- Estimativa ---------------------------------------------------#
# y_tk = [0, 12, 24, 36]

z = np.array([[lmag2(j, i) for j in x] for i in y])
# z = np.array([[86.2639777541, 91.7085907336, 92.6752320789,  95.3093500985, 97.608992404],
#              [88.7806137049, 88.8802508711, 89.9631424714, 90.4234966701, 92.7750794607],
#              [79.4089316549, 85.5515584599, 90.3337106968, 92.3013136073, 95.1794977926],
#              [77.1505598149, 81.5162093743, 87.3006637892, 90.2343727288, 90.291782772]])
...
# ax.set_ylabel("Molhamento (h)")
# ax.set_zlabel("Lesão (cm^2)")
# ax.zaxis.set_major_locator(LinearLocator(10))
# ax.zaxis.set_major_formatter(FormatStrFormatter('%.02f'))
